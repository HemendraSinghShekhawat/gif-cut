...
export class videoProcessor {
    ...
    computeFrames() {
        ...
        if (context) {
            ...
            this.allRenderedFrames[`${currentFrameIndex}`] =
                this.canvasElement.toDataURL("image/png");
        }
    }
    timerCallback() {
    ...
    renderFrame(image) {
        if (context) {
            console.log(image, 0, 0, this.canvasElement.width, this.canvasElement.height);
            context.drawImage(image, 0, 0, this.canvasElement.width, this.canvasElement.height);
        }
    }
    playFrameByFrame() {
